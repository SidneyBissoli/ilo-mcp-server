 ...
 * chamada e apontou cinco ferramentas quebradas, uma delas aqui: `ilo_get_data`
 * falha em 55% das chamadas. Diagnosticar a pior delas, no senado, custou vinte
 * minutos de chamadas manuais porque a telemetria dizia QUE falhou e nao DIZIA
 * por que -- e a primeira hipotese estava errada. Atencao a um limite que vale
 * aqui tambem: erro de validacao de esquema NAO chega a esta camada; o SDK o
 * responde antes do handler, e a chamada nao e contada nem como chamada nem
 * como erro.
 *
 * Por que nao gravar os argumentos. O dado que este servidor serve e publico; a
 * PERGUNTA nao e. O registro ja guarda pais e organizacao de rede, e parte dos
 * parametros e texto livre (`query` do `ilo_search_indicators`), onde cabe
 * qualquer coisa que a pessoa digitou. E ligar isso mudaria o que o servico
 * coleta sem mudar o que a pagina publica diz que ele coleta.
 *
 ...
 *
 * As mensagens deste servidor sao em INGLES e nascem de `IlostatUserError` /
 * `IlostatUpstreamError`. Sete delas passam pela guarda em `call-shape.test.ts`.
 */
